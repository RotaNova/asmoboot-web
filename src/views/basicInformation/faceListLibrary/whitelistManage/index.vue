 ...
 * @LastEditors: LSC
 * @LastEditTime: 2021-12-21 13:43:02
-->
<template>
    <section class="roleManage">
        ...
        <common-add-model ref="commonModel" :title="title" @refresh="getFaceListItemPageList" />
        <common-update-model
...
export default {
    ...
    data() {
        return {
            importContent: '白名单人员导入模板',
            addType: 'WHITE', // 表示白名单
            TYPE_PEOPLE_LIST_WHITE: 1, //1 代表白名单
            title: '',
    ...
    methods: {
        ...
        handleExportAll() {
            ...
            let params = {
                personType: this.TYPE_PEOPLE_LIST_WHITE
            }
            exportAllFaceList(params, this.TYPE_PEOPLE_LIST_WHITE)
                .then((res) => {
        ...
        handleExportChooseFaceList() {
            if (this.deleteid && this.deleteid.length > 0) {
                ...
                exportFaceList(params, this.TYPE_PEOPLE_LIST_WHITE)
                    .then((res) => {
                        this.$message.success('后台导出中，稍后可在消息中心下载，请勿重复点击')
                    })
                    .catch((e) => {})
        ...
        uploadFile() {
            ...
            this.$refs.addImport.toChooseApi(this.addType, this.TYPE_PEOPLE_LIST_WHITE) // 1 代表白名单
            setTimeout(() => {
        ...
        getFaceListItemPageList(obj) {
            ...
            API.getFaceListItemPageListWhile(params)
                .then((res) => {
                    res.records.forEach((record) => {
                        record.isShowCode = false
                        record.isShowPhone = false
                        // if (record.faceGroupNameList) {
                        //     if (record.faceGroupNameList.length === 1) {
                        //         record.reverseArray = record.faceGroupNameList
                        //     } else {
                        //         record.reverseArray = this.reverseArray(record.faceGroupNameList)
                        //     }
                        // } else {
                        //     record.reverseArray = ['暂无分组']
                        // }
                    })
