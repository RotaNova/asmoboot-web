 ...
 * @LastEditors: LSC
 * @LastEditTime: 2021-08-13 11:13:38
-->
<template>
    <div>
...
export default {
    ...
    computed: {
        ...mapGetters(['SET_SCENES_CONFIG_LIST', 'SET_SCENES_DEPLOY']) // res res.re SET_SCENES_CONFIG_LIST
    },
    watch: {
        SET_SCENES_CONFIG_LIST() {
            this.tableData = this.SET_SCENES_CONFIG_LIST.records
            this.ipagination.total = this.SET_SCENES_CONFIG_LIST.total
        }
    },
    methods: {
        success() {
        ...
        handleSubmit(res) {
            ...
            // this.SET_SCENES_DEPLOY.forEach((item, index) => {
            //     property.forEach((key) => {
            //         if (key.isChecked) {
            //             // 全等去重
            //             if (key.scenesId === item.scenesId && key.delayTime == item.delayTime) {
            //                 this.SET_SCENES_DEPLOY.splice(index, 1)
            //             }
            //         }
            //     })
            // })
            let arr = [...this.SET_SCENES_DEPLOY, ...property]
            let newArr = JSON.parse(JSON.stringify(this.SET_SCENES_DEPLOY))
            for (let item of arr) { //循环json数组对象的内
                let flag = true; //建立标记，判断数据是否重复，true为不重复
                for (let key of newArr) { //循环新数组的内容
                    if (key.scenesId === item.scenesId && key.delayTime == item.delayTime) { //让json数组对象的内容与新数组的内容作比较，相同的话，改变标记为false
                        flag = false;
                    }
                }
                if (flag) { //判断是否重复
            ...
            this.$store.commit('SET_SCENES_DEPLOY', newArr)
        ...
        getScenesConfigList() {
            ...
            // ?传分页
            this.$store
                .dispatch('GET_SCENES_CONFIG_LIST', params)
                .then(() => {
                    this.loading = false
                })
                .catch(() => { })
