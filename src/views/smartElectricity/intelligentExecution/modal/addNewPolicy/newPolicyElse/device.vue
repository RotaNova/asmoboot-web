 ...
 * @LastEditors: LSC
 * @LastEditTime: 2021-08-13 11:05:03
-->
<template>
    <div>
...
export default {
    ...
    computed: {
        ...mapGetters(['SET_DEVICE_ElSE_CONDITIONLIST', 'SET_DEVICEPOLICYELSE']) // res res.re
    },
    watch: {
        SET_DEVICE_ElSE_CONDITIONLIST() {
            this.tableData = this.SET_DEVICE_ElSE_CONDITIONLIST.records
            this.ipagination.total = this.SET_DEVICE_ElSE_CONDITIONLIST.total
        }
    },
    methods: {
        success() {
            this.ipagination.current = 1
            this.getDeviceElseConditionList()
        },
        handleCancels() {
            this.$emit('close')
        },
        ...
        handleSubmit(res) {
            ...
            // this.SET_DEVICEPOLICYELSE.forEach((item, index) => {
            ...
            //                 this.SET_DEVICEPOLICYELSE.splice(index, 1)
            //             }
            //         }
            //     })
            // })
            // !根据id相同和属性相同进行一次去重操作
            let arr = [...this.SET_DEVICEPOLICYELSE, ...property]
            let newArr = JSON.parse(JSON.stringify(this.SET_DEVICEPOLICYELSE))
            ...
            this.$store.commit('SET_DEVICEPOLICYELSE', newArr)
        ...
        getDeviceElseConditionList() {
            ...
            // ?传分页
            this.$store
                .dispatch('GET_DEVICE_ElSE_CONDITIONLIST', params)
                .then(() => {
                    this.loading = false
                })
                .catch(() => { })
