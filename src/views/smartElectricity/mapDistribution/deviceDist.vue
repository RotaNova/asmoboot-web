<template>
    <div class="map-box">
        ...
        <div class="Unknown_device">未知位置设备: {{ uncount ? uncount : 0 }}个</div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        getProjectUnknownLocation() {
            API.getUnknownLocationDeviceCount().then((res) => {
                this.uncount = res
            })
        },
        // 树形数据平铺
        ...
        findProvinceProject() {
            ...
            API.listProvinceDeviceStatistic(parame).then((res) => {
                res.forEach((item) => {
                    if (this.addressMap[item.postCode]) {
                        let num = this.addressMap[item.postCode].label
                        let localtion = this.addressMap[item.postCode].center.split(',')
                        let arr = []
        ...
        getPointData() {
            ...
            API.listDeviceDistributions(parame).then((res) => {
                res.forEach((item) => {
        ...
        findCityProject() {
            ...
            API.listCityDeviceStatistic(parame).then((res) => {
                res.forEach((item) => {
                    if (this.addressMap[item.postCode]) {
                        let num = this.addressMap[item.postCode].label
                        let localtion = this.addressMap[item.postCode].center.split(',')
                            ...
                            info.push(
                                `<div style="padding:7px 0px 0px 0px;width:400px;color:#555;">
                                    <div class="mt10" style="display:flex;justify-content: space-between;">
                                        <p>设备名: ${numObj.nickName}</p>
                                        <p>
                                            <span class="${
                                                numObj.status == 1 ? 'map_online' : 'map_onoutline'
                                            }" style="width:10px;height:10px;border-radius: 50%;;margin-right:2px;display:inline-block;"></span>
                                            <span>${numObj.status_dictText}</span>
                                        </p>
                                    </div>
                                   <p class="mt10">设备编码: ${numObj.deviceName}</p>
                                   <p class="mt10">设备序号: ${numObj.deviceCode}</p>
                                   <p class="mt10">负责人: ${
                                       numObj.manager ? numObj.manager : ''
                                   }</p>
                                    <p class="mt10">联系方式: ${
                                        numObj.contactInfo ? numObj.contactInfo : ''
                                    }</p>
