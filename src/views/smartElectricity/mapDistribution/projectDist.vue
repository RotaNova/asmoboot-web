<template>
    <div class="map-box">
        ...
        <div class="Unknown_device">未知位置网关: {{ uncount ? uncount : 0 }}个</div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        getProjectUnknownLocation() {
            API.getUnknownLocationGatewayCount().then((res) => {
                this.uncount = res
            })
        },
        // 树形数据平铺
        ...
        findProvinceProject() {
            let _this = this
            API.listProvinceGatewayStatistic().then((res) => {
                res.forEach((item) => {
        ...
        getPointData() {
            ...
            API.listGatewayDistributions(parame).then((res) => {
                res.forEach((item) => {
        ...
        findCityProject() {
            let _this = this
            API.listCityGatewayStatistic().then((res) => {
                res.forEach((item) => {
                    if (this.addressMap[item.postCode]) {
                        let num = this.addressMap[item.postCode].label
                        let localtion = this.addressMap[item.postCode].center.split(',')
