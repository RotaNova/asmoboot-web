...
-->
<template>
<a-modal style="z-index: 10" title="通行权限下发" :visible="showMove" width="1000px" centered cancelText="关闭" okText="下发" @cancel="handleCancels" v-if="deleteDom" @ok="issuedEquipment">
    <div class="catchDev">
        <div class="left">
            <a-input-search style="margin-bottom: 8px" placeholder="请输入关键字" @change="onChange" />
            <a-spin :spinning="isSpinningTree">
                <a-tree class="my-tree" :expanded-keys="expandedKeys" :auto-expand-parent="autoExpandParent" :tree-data="gData" show-icon :selectedKeys="selectedata" @expand="onExpand" @select="onSelect" v-model="checkeds" :checkStrictly="true" :replaceFields="replaceFields">
                    <template slot="title" slot-scope="{ groupName, groupCode, totalPersonNum, choosePersonNum }">
                        ...
                        <span v-if="totalPersonNum">{{ `( ${choosePersonNum}/${totalPersonNum} )` }}
                        </span>
        ...
        <div class="right">
            <a-input-search v-show="selectedKeys" style="margin-bottom: 8px" placeholder="请输入关键字" v-model="searchDevice" @search="getCameraItemPageList(1)" />
            <a-empty v-show="!selectedKeys" description="请先选择一个分组" style="margin-top: 68px" />
                ...
                <a-spin :spinning="isSpinningList">
                    <a-list :data-source="listData" class="list">
                        <a-list-item slot="renderItem" slot-scope="item" class="dev-list">
                            ...
                            <div>
                                <a-checkbox v-model="item.access" @click="handleClickItem(item)"></a-checkbox>
                            </div>
                        </a-list-item>
                    </a-list>
                </a-spin>
                <!--  小型分页组件 class="page" v-show="total !== 0"-->
                <smallSecondPage v-show="isShow" ref="smallPagination" class="pages" @success="getCameraItemPageList(...deviceGroupId)" />
            </div>
        </div>
    </div>
</a-modal>
